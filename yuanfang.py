...
check_time = datetime(1,1,1)
tmp_time = datetime(1,1,1)
...
def check_status(status_id, status_title, status_time):
    if re.search('@zhuxi0511', status_title) is not None and status_time > check_time:
        tmp_time = status_time
...
def check_reply(status_id):
    ...
    for reply in json_reply:
        ...
        if re.search('@zhuxi0511', reply['text']) is not None and reply_time > check_time:
            if reply['user']['uid']:
